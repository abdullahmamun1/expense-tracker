...
type CategoryBreakdownRangeProps = {
...
export function CategoryBreakdownRange({ byCategory, rangeLabel }: CategoryBreakdownRangeProps) {
  const expenses = byCategory.filter((entry) => entry.type === "EXPENSE");
  const income = byCategory.filter((entry) => entry.type === "INCOME");

  return (
    <div className="flex flex-col gap-8">
      <BreakdownGroup title={`Expenses — ${rangeLabel}`} entries={expenses} />
      <BreakdownGroup title={`Income — ${rangeLabel}`} entries={income} />
    </div>
  );
}
